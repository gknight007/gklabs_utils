...
def usage():
    sys.stderr.write("Usage: %s -j <job_name> -k <param_name> -v <param_value> -d <description> " % sys.argv[0])
    sys.stderr.write("[-h <jenkins_host>] [-p <jenkins_port>] [-u <jenkins_user>] ")
    sys.stderr.write("[-w <jenkins_password>]\n")

def updateConfig(jobName, paramName, paramValue, description):
    global user
    global passwd
    global host
    global port
    ...
    paramsElement = configXmlTree.find('.//parameterDefinitions')
    stringPropElement = ElementTree.SubElement(paramsElement, 'hudson.model.StringParameterDefinition')

    nameElement = ElementTree.SubElement(stringPropElement, 'name')
    nameElement.text = paramName

    descriptionElement = ElementTree.SubElement(stringPropElement, 'description')
    descriptionElement.text = description

    defaultValueElement = ElementTree.SubElement(stringPropElement, 'defaultValue')
...
def main():
    ...
    global host
    global port
    jobName, paramName, paramValue, description = None, None, None, None

    try:
        opts, args = getopt.getopt(sys.argv[1:], 'u:w:h:p:j:k:v:d:', [])
    ...
    if jobName == None or paramName == None or paramValue == None:
        usage()
        exit(1)
    updateConfig(jobName, paramName, paramValue, description)
